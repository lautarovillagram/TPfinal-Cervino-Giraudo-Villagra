...
public class UsuarioExperto extends Usuario {
	...
	public void opinar(Muestra muestra, String especie) {
		// Busco si hay opiniones de expertos iguales a la mia en la muestra antes de opinar
		if (muestra.opinionesDeExpertos().stream().filter(o -> o.getEspecie().equals(especie)).count() > 0) {
			// En caso de que haya, la muestra queda verificada
